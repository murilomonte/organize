...
class $SubtasksTable extends Subtasks with TableInfo<$SubtasksTable, Subtask> {
  ...
  Subtask map(Map<String, dynamic> data, {String? tablePrefix}) {
    ...
    return Subtask(
      ...
      status: $SubtasksTable.$converterstatus.fromSql(
        attachedDatabase.typeMapping.read(
          DriftSqlType.string,
          data['${effectivePrefix}status'],
        )!,
...
class Subtask extends DataClass implements Insertable<Subtask> {
  ...
  Map<String, Expression> toColumns(bool nullToAbsent) {
    ...
    {
      map['status'] = Variable<String>(
        $SubtasksTable.$converterstatus.toSql(status),
      );
    }
    map['created_at'] = Variable<DateTime>(createdAt);
    map['updated_at'] = Variable<DateTime>(updatedAt);
...
class SubtasksCompanion extends UpdateCompanion<Subtask> {
  ...
  Map<String, Expression> toColumns(bool nullToAbsent) {
    ...
    if (status.present) {
      map['status'] = Variable<String>(
        $SubtasksTable.$converterstatus.toSql(status.value),
      );
    }
    if (createdAt.present) {
      map['created_at'] = Variable<DateTime>(createdAt.value);
...
final class $$GroupsTableReferences
  ...
  static MultiTypedResultKey<$TasksTable, List<Task>> _tasksRefsTable(
    ...
    db.tasks,
    aliasName: $_aliasNameGenerator(db.groups.id, db.tasks.group),
  );
  ...
  $$TasksTableProcessedTableManager get tasksRefs {
    final manager = $$TasksTableTableManager(
      $_db,
      $_db.tasks,
    ).filter((f) => f.group.id.sqlEquals($_itemColumn<int>('id')!));
                ...
                return [
                  if (tasksRefs)
                    await $_getPrefetchedData<Group, $GroupsTable, Task>(
                      currentTable: table,
                      referencedTable: $$GroupsTableReferences._tasksRefsTable(
                        db,
                      ),
...
final class $$TasksTableReferences
  ...
  static $GroupsTable _groupTable(_$AppDatabase db) =>
      db.groups.createAlias($_aliasNameGenerator(db.tasks.group, db.groups.id));

  $$GroupsTableProcessedTableManager get group {
    final $_column = $_itemColumn<int>('group')!;
  ...
  static MultiTypedResultKey<$SubtasksTable, List<Subtask>> _subtasksRefsTable(
    ...
    db.subtasks,
    aliasName: $_aliasNameGenerator(db.tasks.id, db.subtasks.task),
  );
...
class $$TasksTableTableManager
    extends
        RootTableManager<
          ...
          Task,
          PrefetchHooks Function({bool group, bool subtasksRefs})
        > {
    ...
    : super(
        TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              ...
              .toList(),
          prefetchHooksCallback: ({group = false, subtasksRefs = false}) {
            return PrefetchHooks(
              db: db,
...
final class $$SubtasksTableReferences
  ...
  static $TasksTable _taskTable(_$AppDatabase db) =>
      db.tasks.createAlias($_aliasNameGenerator(db.subtasks.task, db.tasks.id));
...
class $$SubtasksTableTableManager
    extends
        RootTableManager<
          ...
          Subtask,
          PrefetchHooks Function({bool task})
        > {
    ...
    : super(
        TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              ...
              .toList(),
          prefetchHooksCallback: ({task = false}) {
            return PrefetchHooks(
              db: db,
